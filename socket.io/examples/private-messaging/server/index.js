...
const io = require("socket.io")(httpServer, {
  cors: {
    origin: ["http://localhost:8080","http://192.168.86.27:8080","http://172.20.10.2:8080","http://10.35.122.97:8080","http://10.35.127.32:3000"],
  },
});
...
function scanForKeyword(msg, username){
...
async function sentimentAnalyse(msg, username){
  // const url = require('url')
  var result = {}
  const span = tracer.scope().active()
  span.setTag('username', username)

  // first remove sin (datadog instrumentation)


  // const toBeEvaluated = removeSinFromMessage(msg)
  // let res =  await callSentimentApp(toBeEvaluated)

  const toBeEvaluated = tracer.trace('custom', { resource: 'removeSinFromMessage'}, () => {
    return removeSinFromMessage(msg)
  })

  let res = await tracer.trace('custom', { resource: 'callSentimentApp'}, () => {
    return callSentimentApp(toBeEvaluated)
  })
...
scanForKeyword = tracer.wrap('custom', { resource: 'scanForKeyword' }, scanForKeyword)
sentimentAnalyse = tracer.wrap('custom', { resource: 'sentimentAnalyse'}, sentimentAnalyse)
// callSentimentApp = tracer.wrap('custom', { resource: 'callSentimentApp'}, callSentimentApp)
...
io.on("connection", (socket) => {
  ...
  socket.on("private message", ({ content, to }) => {
    ...
    results = scanForKeyword(content, socket.username)
    if (results == "sin") {
      console.log("Sin was mentioned")
      const everything = async () => {
        sa_results = sentimentAnalyse(content, socket.username)
        return sa_results
      }
